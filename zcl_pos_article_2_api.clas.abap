class ZCL_POS_ARTICLE_2_API definition
  public
  final
  create public .
...
private section.
  ...
  class-methods MAP_ARTICLE_DATA
    ...
    returning
      value(RS_ARTICLE) type ZXI_MAGENTO_PRODUCT_EXTERNAL_M
    raising
      ZCX_POS_EXCEPTION .
...
CLASS ZCL_POS_ARTICLE_2_API IMPLEMENTATION.
...
* The body which is created depends on whether the article already exists in the global scope
    DATA(ls_article_proxy) = map_article_data( EXPORTING is_article_price = is_article_price iv_bundle = rs_status-is_bundle iv_exists = lv_exists_previously ).


    DATA(lr_magento_article) = NEW zxi_co_si_os_magento_product_e( ).
    TRY.
        lr_magento_article->si_os_agento_product_external(
          EXPORTING
            output = ls_article_proxy
          IMPORTING
            input  = DATA(ls_response) ).
      CATCH cx_ai_system_fault INTO DATA(lr_exception).
        DATA(out) = cl_demo_output=>new( )->begin_section( 'Exception' ).
        out->write_text( lr_exception->get_text( ) ).
        out->display( ).
    ENDTRY.
...
*    CALL METHOD /ui2/cl_json=>deserialize( EXPORTING json = lv_body CHANGING data = rt_children ).
  ...
  METHOD DISPLAY_STATUS_ALV.
    ...
    SET HANDLER zcl_pos_article_2_api=>on_alv_double_click FOR mr_events.
...
*    ENDIF.
  ...
  METHOD map_article_data.
    ...
    DATA:
      ls_custom_attr TYPE zxi_zpos_mag_cust_attr_sty.

    rs_article-magento_product_external_messa-product-sku = is_article_price-matnr. SHIFT rs_article-magento_product_external_messa-product-sku LEFT DELETING LEADING '0'.
...
*      APPEND INITIAL LINE TO rs_article-magento_product_external_messa-product-extension_attributes-website_ids ASSIGNING FIELD-SYMBOL(<ls_website_id>).
*      <ls_website_id> = ls_website-website_id.

      rs_article-magento_product_external_messa-product-visibility = 1.      "9 Will be replace with 0 later - after the conversion to JSON

      rs_article-magento_product_external_messa-product-attribute_set_id = ls_website-attr_set_id.



      APPEND INITIAL LINE TO rs_article-magento_product_external_messa-product-extension_attributes-category_links-item ASSIGNING FIELD-SYMBOL(<ls_category_link>).
    ...
    IF iv_bundle = abap_true.
      ...
      LOOP AT is_article_price-bom INTO DATA(ls_child).
        ...
        APPEND INITIAL LINE TO rs_article-magento_product_external_messa-product-extension_attributes-bundle_product_options-item ASSIGNING FIELD-SYMBOL(<ls_bundle_child>).
        ...
        APPEND INITIAL LINE TO <ls_bundle_child>-product_links-item ASSIGNING FIELD-SYMBOL(<ls_prod_link>).
    ...
    rs_article-magento_product_external_messa-product-name = is_article_price-description && '-' && sy-uzeit.
    rs_article-magento_product_external_messa-product-price = is_article_price-eff_price.
    rs_article-magento_product_external_messa-product-status = 1.
    rs_article-magento_product_external_messa-product-type_id = COND #( WHEN iv_bundle = abap_true THEN zcl_pos_util=>co_bundle ELSE zcl_pos_util=>co_simple ).

    rs_article-magento_product_external_messa-product-weight = is_article_price-gross_weight.
*    rs_article-magento_product_external_messa-product-extension_attributes-stock_item-qty = 100.
    ...
    IF NOT is_article_price-country_origin IS INITIAL.
      ...
      APPEND ls_custom_attr TO rs_article-magento_product_external_messa-product-custom_attributes-item.
    ENDIF.
