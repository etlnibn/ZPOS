class ZCL_POS_SALES_SET definition
  public
  final
  create public .
...
public section.
  ...
  class-methods CREATE_SALES_SET_IDOC
    importing
...
private section.
  ...
  class-methods GET_SALES_SET_DISCOUNT_ID
    importing
      !IV_EAN11 type EAN11
...
CLASS ZCL_POS_SALES_SET IMPLEMENTATION.


  METHOD create_sales_set_idoc.
    ...
    DELETE lt_article_price WHERE bom IS INITIAL.
...
*------------------------------- CONTROL RECORD ------------------------------------
      ...
      LOOP AT GROUP ls_article_price ASSIGNING FIELD-SYMBOL(<ls_article_price>).
        ...
        CLEAR: ls_idoc_data, ls_offer_header, ls_offer_item.
        lv_counter += 1.
...
*------------------------------- ZPOS_OFFER_HEADER ------------------------------------
        ...
        ls_offer_header-vk_offer_id = get_sales_set_discount_id( iv_ean11 = <ls_article_price>-ean11 iv_matnr = <ls_article_price>-matnr iv_valid_to = <ls_article_price>-datbi ).
        ...
        ls_offer_header-discount_method = zcl_pos_util=>co_offer_discount_method-total_price.
        ls_offer_header-discount_value = <ls_article_price>-eff_price.
...
*------------------------------- ZPOS_OFFER_ITEM_01  -----------------------------------
        SORT ls_article_price-bom BY bom_ean11 ASCENDING.
        LOOP AT ls_article_price-bom INTO DATA(ls_bom).

          IF  ls_bom-bom_ean11 IS INITIAL.
            ms_textid-msgid = zcl_pos_util=>co_pos_msgid.
            ms_textid-msgno = '100'.
            ms_textid-attr1 = ls_article_price-matnr.
            ms_textid-attr2 = ls_bom-idnrk.           " BOM Article
...
*------------------------------- Single Article -----------------------------------
          ...
          ls_offer_item-itemtype  = '0'.       "Always Article
          ls_offer_item-itemid    = ls_bom-bom_ean11.
          ls_offer_item-articleid = ls_bom-idnrk.
          ls_offer_item-quantity  = ls_bom-menge.
          ls_offer_item-discount_distribution = co_bool_01_true.
          ls_offer_item-discount_calculation  = co_bool_01_true.
...
*            MESSAGE i000(zpos) WITH 'IDoc Created' && ls_edidc-docnum.
          ...
          CLEAR: ls_idoc_data, lt_edidd, ls_edidc-docnum, ls_offer_header, ls_offer_item, lv_parent_segnum, lv_segnum.
...
*          MESSAGE i000(zpos) WITH 'IDoc Created' && ls_edidc-docnum.
  ...
  METHOD get_sales_set_discount_id.
...
* for offers for Viking
    SELECT SINGLE * FROM zpos_vk_offer
           INTO @DATA(ls_vk_offer)
            ...
            AND     category   = @zcl_pos_util=>co_offer_category-sales_set.
...
* Add to table
        ...
        ls_new_offer-category     = zcl_pos_util=>co_offer_category-sales_set.
